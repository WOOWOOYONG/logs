...
3. 可以開始在裡面存放`markdown`檔案了
   ![Nuxt Content notes 資料夾結構示範](../../assets/images/notes/19/nuxtcontent1.png)
...
<template>
  <ul>
    ...
    >
      <NuxtLink :to="note._path" class="block text-2xl font-bold duration-300 hover:-translate-x-1">
        {{ note.title }}
      </NuxtLink>
...
3. 到`markdown`檔案內裡面直接引入
   ![在 markdown 中引入 Vue 元件的示範](../../assets/images/notes/19/nuxtcontent2.png)
